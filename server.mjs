...
import { registerContractIntelligence, CONTRACT_PROFILES, FIGURE_LINKS, FUND_TAGS, LOBBY_CLIENT_TICKERS, agencySignalScore, awardNoveltyScore, computeGovernmentDependencyScore, computeContractEventSignal, buildGovernmentMoneyTrail, contractCausality } from "./src/server/contract-intelligence.mjs";
import { registerPredictionsApi, predictionScorecardHandler, predictionVerifyHandler, predictionListHandler, predictionRecordHandler, autoRecordSignalPredictions } from "./src/server/predictions-api.mjs";
import { registerAuthEmail, authSignup, authLogin, authLogoutApi } from "./src/server/auth-email.mjs";
...
registerContractIntelligence({
  ...
  mkdir,
  DATA_DIR,
  TRENDING_TOPICS_FILE,
  CONTRACT_WATCH_FILE,
  quoteSnapshot,
  computeLegislativeMomentum
});
registerPredictionsApi({
  sendJson,
  readJson,
  quoteSnapshot,
  POLICY_BILLS,
